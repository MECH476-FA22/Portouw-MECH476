...
#not sure, but it would be possible to check and see #if the package is present in the user's workspace, #and if not, install it if necessary. 
...
#Note: Working directory is /Portouw-MECH476/Homework/Chapter_3. 
#csv of interest is in #/Portouw-MECH476/Data/ftc_o3.csv
fc_ozone <- readr::read_csv("../../Data/ftc_o3.csv")%>%
  dplyr::select(sample_measurement,
                datetime)%>%
  dplyr::filter(!is.na(sample_measurement),
                       !is.na(datetime))%>%
  dplyr::rename(ozone_ppm_hourly = sample_measurement)
...
NA_removed <- nrow(readr::read_csv("../../Data/ftc_o3.csv"))-
  nrow(fc_ozone)
```
...
top_oz_hourly <- dplyr::slice_max(.data = fc_ozone,
                                  order_by = ozone_ppm_hourly,
                                  n = 10)
```
...
bot_oz_hourly <- dplyr::slice_min(.data= fc_ozone,
                                  ozone_ppm_hourly,
                                  n = 10)
```
...
The highest values seem to occur around 8 to 10 pm,
while the lowest values occur around 7-11 am.
...
hi_oz_time <- top_oz_hourly[1,2]
# extract date/time of lowest ozone concentration
low_oz_time <- bot_oz_hourly[1,2]
```
...
ozone_ppm <- fc_ozone$ozone_ppm_hourly
...
#final equation:
ozoneugm3 <- function(ozone_ppm){
  ozoneugm <- 1000*(((ozone_ppm)*(mol_wt_oz)*(C_to_K)*air_pressure)/(R*(C_to_K+C)*air_pressure))
  return(ozoneugm)
}
dplyr::mutate(.data = fc_ozone, ozone_ppm_hourly = ozoneugm3(ozone_ppm))%>%
  rename(ozone_umg3 = ozone_ppm_hourly)
