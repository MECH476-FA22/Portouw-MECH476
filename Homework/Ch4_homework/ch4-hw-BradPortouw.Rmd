...
fc_ozone <- readr::read_csv("../../Data/ftc_o3.csv")%>%
...
ggplot(data = fc_ozone, aes(x= datetime, y= ozone_ppm)) +
  geom_point()
```
...
Below is a plot using the plot() function provided in base r. the basic formating is much worse, but it uses basic vectors for data rather than utilizing columns from a particular data frame. 

```{r ozone-time-base-r}
X <- fc_ozone$datetime
Y <- fc_ozone$ozone_ppm
plot(X,Y)
```
...
ozone_ppm_vs_time <- ggplot(data = fc_ozone, aes(x= datetime, y= ozone_ppm))
```
...
Observing ozone_ppm_vs_time there are zero layers.If the geom is added during the creation of the ggplot, like in question 1 there is 1 layer. The + operator acts much like a pipe to another function, however the syntax used in question 4 must have added the outputs of the two functions together. geom_point() likely has a workaround where it will alter the plot while technically not being an ongoing change to the object. 
# Question 6: theme
...
dates and times, we will instead focus on examining the ozone measurements that
...
fc_ozone_high <- dplyr::filter(.data=fc_ozone, ozone_ppm > 0.070)
# time series of high ozone measurements
high_ozone_ppm_vs_time <- ggplot(data = fc_ozone_high, aes(x= datetime, y=ozone_ppm))+
  geom_point() +
  ggtitle("Hourly Ozone Measurements in Fort Collins, CO") +
  ylab("Ozone Concentration (ppm)") +
  xlab('Date of Measurement')
high_ozone_ppm_vs_time
```
...
Winter sees less ozone ppm versus summer. The plot that only includes ppm over 0.070 only contains values between the months of June to September which are some of the hottest months in Fort Collins.
...
percent_exceed0.070 <- (length(fc_ozone_high$ozone_ppm)/length(fc_ozone$ozone_ppm))*100
```
...
I'm currently a junior and do not have a senior project, however it is clear that these extensions provide ways of conveying information in more practical and exciting ways.
differing colors, motions, and figures can bring attention to one's data in order to more clearly convey what was observed during testing. 
The whole goal of making plots is to convey information, and having more options can only be helpful
\newpage
